...
def train_interpet(
    ...
    for epoch in range(n_epochs):
        ...
        for _ in range(1):  # 每轮优化因果图1次
            ...
            # 只有在启用时才计算约束
            if use_acyclicity:
                loss_c = lambda_c * acyclicity_constraint(P)
            else:
                loss_c = torch.tensor(0.0).to(device)
                
            if use_sparsity:
                loss_s = lambda_s * torch.norm(P, p=1)
            else:
        ...
        if epoch % 5 == 0:
            ...
            # 1. 计算验证集准确率 (作为无反事实时的指标)
            if X_val is not None:
                with torch.no_grad():
                    logits_val = predictor(torch.FloatTensor(X_val).to(device))
                    preds_val = torch.argmax(logits_val, dim=1).cpu().numpy()
                    val_acc = accuracy_score(y_val, preds_val)
            
            # 2. 如果启用了反事实验证，计算 Dispo
            if use_counterfactual:
                with torch.no_grad():
                    _, G_hat = causal_module(torch.FloatTensor(X_train).to(device))
                
                # 采样少量测试样本生成反事实
                # 30% 采样可能太慢，ablations 时通常用少一点，这里保持原逻辑或稍微减少
                sample_size = min(200, int(n_samples * 0.2)) # 为了 ablation 速度，限制最大200
                test_indices = np.random.choice(n_samples, sample_size, replace=False)
                # test_indices = np.random.choice(n_samples, min(50, n_samples), replace=False)
                ...
                if len(counterfactuals) > 0:
                    dispo = compute_discriminative_power(
                        ...
                        X_train[test_indices], # Use sampled test instances
                        X_train,
                        y_train,
                        predictor,
                        device
            ...
            # ===== 早停判断 =====
            # 优先使用 dispo，如果没开 dispo 则使用 val_acc
            ...
            if epoch % 10 == 0:
                print(f"Epoch {epoch:3d} | Loss_pred: {epoch_loss_pred/5:.4f} | "
                      f"Loss_g: {epoch_loss_graph:.4f} | "
                      f"Dispo: {dispo:.4f} | Val Acc: {val_acc:.4f}")
...
if __name__ == "__main__":
    ...
    (X_train_full, y_train_full), (X_test, y_test), feature_names, cat_mask, scaler = load_data(
        TRAIN_CSV, TEST_CSV
    )
    ...
    configs = [
        # {"name": "Full Model", "acyclicity": True, "sparsity": True, "counterfactual": True},
        {"name": "No Acyclicity", "acyclicity": False, "sparsity": True, "counterfactual": True},
        {"name": "No Sparsity", "acyclicity": True, "sparsity": False, "counterfactual": True},
        {"name": "No Counterfactual", "acyclicity": True, "sparsity": True, "counterfactual": False},
    ]
    ...
    for cfg in configs:
        ...
        ckpt_dir = f"/workspace/causal_discovery/result/syn_dataset/{n_samples}samples_{n_features}features/ablation/{name.replace(' ', '_')}"
        
        if cfg['counterfactual']:
            curr_X_val, curr_y_val = None, None
            curr_X_train=X_train_full
            curr_y_train=y_train_full
        else:
            curr_X_val, curr_y_val = X_val, y_val
            curr_X_train=X_train
            curr_y_train=y_train

        causal_module, predictor, best_dispo = train_interpet(
            X_train=curr_X_train, y_train=curr_y_train,
            X_val=curr_X_val, y_val=curr_y_val,
            n_epochs=1000, # 减少轮次以加快实验速度
            checkpoint_dir=ckpt_dir,
            use_acyclicity=cfg['acyclicity'],
            use_sparsity=cfg['sparsity'],
            use_counterfactual=cfg['counterfactual']
        )
        
        res = test_interpet(
            f"{ckpt_dir}/model.pth",
            X_test, y_test,
    ...
    # 保存结果
    with open(f'/workspace/causal_discovery/result/syn_dataset/{n_samples}samples_{n_features}features/ablation_results.json', 'w') as f:
        json.dump(final_results, f, indent=2)
    
    print("\nAblation Study Completed. Results:")
